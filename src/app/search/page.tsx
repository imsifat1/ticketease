...
import { ArrowRight, Calendar, SlidersHorizontal, Loader2 } from 'lucide-react';
...
function SearchResults() {
  ...
  const { isLoggedIn, setLoginDialogOpen } = useContext(AuthContext);
  ...
  const handleRouteSelect = (route: BusRoute) => {
    if (isLoggedIn) {
      ...
      setPendingRoute(route); // Save the route selection
      setLoginDialogOpen(true); // Open the global login dialog
    }
  };
  
  // Effect to handle booking after successful login
  // This is a new pattern since the dialog is now global.
  // We check if there's a pending route and if the user is now logged in.
  useMemo(() => {
